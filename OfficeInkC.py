...
class font:
    ...
    SleepFont_sub : FreeTypeFont
    SleepFont_foot :FreeTypeFont
    BatteryPercentage :FreeTypeFont
...
screen.epd_found = False
try:
    from waveshare_epd import epd7in3f
    screen.epd_found = True
except:
    print("No e-Paper screen found...")
...
def get_dashboard_config_data(file_path:str):
    ...
    font.BatteryPercentage = ImageFont.truetype("fonts/SF-Compact-Rounded-Medium.otf",22)
...
def welcome_screen(delay_time_in_sec):
    ...
    ImageC = Image.new('RGB', (screen.width, screen.height), white)  # 255: clear the frame
    ...
    if hourglass.curenttime >= 0 and hourglass.curenttime <=6:
        salute_text = "ZZzzz.."
    if hourglass.curenttime >= 6 and hourglass.curenttime <=11:
        salute_text = "Good Morning"
    if hourglass.curenttime > 11 and hourglass.curenttime <=15:
        salute_text = "Good Afternoon"
    if hourglass.curenttime > 15 and hourglass.curenttime <=19:
        salute_text = "Good Evening"
    if hourglass.curenttime > 19 and hourglass.curenttime <22:
        salute_text = "Good Night"


    #t_G, t_G, test_t_w, test_t_h = draw_color.textbbox((0,0),salute_text, font=font.SleepFont)
    ...
    if performance.ip_address == "no_ip":
        ...
        if "wifi-off" in performance.cli:
            net_icon = Image.open(os.path.join(picdir, 'wifi_off_option_c.png'))
        else:
            net_icon = Image.open(os.path.join(picdir, 'wifi_on_c.png'))
        
        gx = 10
        gy = 10
        ImageC.paste(net_icon, (gx,gy), net_icon)
        tx = int(gx + (net_icon.size[0] + 4))
        ty = int(gy + int((net_icon.size[1]/2) - 12))
        if "wifi-off" in performance.cli:
            draw_color.text((tx, ty), performance.ip_address+" WiFi: OFF after this screen", font = font.SleepFont_foot, fill = blue)
        else:
            draw_color.text((tx, ty), performance.ip_address+"@"+performance.host_name+".local", font = font.SleepFont_foot, fill = black)

    welome_icon = Image.open(os.path.join(picdir, 'johansson.png'))

    welcome_string = salute_text + " from OfficeInk"
    t_g, t_g, test_t_w, test_t_h = draw_color.textbbox((0,0),welcome_string, font=font.SleepFont)
    welcome_y = int(test_t_h) + 20
    ...
    draw_color.text((sX, sY), welcome_string, font = font.SleepFont, fill = black)
    ...
    if dashboard.show_power == 1:
        ...
        if battlevel.level >-1: #Checking if battery level can be goten -1 means error
            ...
            if battlevel.state == "Charging":
                if battlevel.level <85:
                    batt_icon = Image.open(os.path.join(picdir, 'Battery_chrging_c.png'))
                else:
                    batt_icon = Image.open(os.path.join(picdir, 'Battery_chrg_full_c.png'))
                btx = 5
                bty = (screen.height) - int(batt_icon.height)
                ImageC.paste(batt_icon, (btx,bty), batt_icon)
            else:
                if battlevel.level > 35:
                    if battlevel.level >= 85:
                        batt_icon = Image.open(os.path.join(picdir, 'Battery_full_c.png'))
                    else:
                        batt_icon = Image.open(os.path.join(picdir, 'Battery_c.png'))
                        batt_load_txt = str(battlevel.level)+"%"
                    btx = 5
                    bty = (screen.height) - int(batt_icon.height)
                    ImageC.paste(batt_icon, (btx,bty), batt_icon)
                    if battlevel.level < 85:
                        t_G, t_G, batt_t_w, batt_t_h = draw_color.textbbox((0,0),batt_load_txt, font=font.BatteryPercentage)
                        btx = btx + int(batt_icon.width/2)
                        btx = (btx - int(batt_t_w/2)) - 4
                        bty = (bty + int(batt_t_h/2)- 4) 
                        draw_color.text((btx, bty), batt_load_txt, font=font.BatteryPercentage, fill = white)
                else:
                        batt_icon = Image.open(os.path.join(picdir, 'Battery_low_c.png'))
                        btx = 5
                        bty = (screen.height) - int(batt_icon.height)
                        bty = bty - 4
                        ImageC.paste(batt_icon, (btx,bty), batt_icon)
            ...
            batt_icon = Image.open(os.path.join(picdir, 'Battery_no_batt_c.png'))
            btx = 5
            bty = (screen.height) - int(batt_icon.height)
            ImageC.paste(batt_icon, (btx,bty), batt_icon)
    else:
    ...
    wakeup_string = "OfficeInk will start in "+str(delay_time_in_sec)+" seconds."
    t_g, t_g, test_t_w, test_t_h = draw_color.textbbox((0,0),wakeup_string, font=font.SleepFont_foot)
    sX = int(screen.middle_w) - int(test_t_w/2)
    sY = int(screen.height - (int(test_t_h) + 5))
    draw_color.text((sX, sY), wakeup_string, font = font.SleepFont_foot, fill = red)
    
    if screen.epd_found == True:
        epd.display(epd.getbuffer(ImageC))
        epd.sleep()
    else:
        ImageC.show()
...
def daily_quotes():
    ...
    if screen.epd_found == True:
        try:
            applog("Daily Quote" ,"Setting screen")
            epd = epd7in3f.EPD()
            epd.init()
            applog("Daily Quote" ,"INIT screen")
            if performance.cli == "noclean":
                applog("Daily Quote" ,"Screen cleaning skipped")
            else:
                applog("Daily Quote" ,"Time to clean the screen")
                epd.Clear()
            screen.height = epd.height
            screen.width = epd.width
            screen.middle_w = screen.width/2
            ...
            applog("Daily Quote" ,"error"+str(e)) 

        except KeyboardInterrupt:
            applog("Daily Quote" ,"ctrl + c received") 
            epd7in3e.epdconfig.module_exit()
            exit()
    else:
            screen.height = 480
    ...
    if dashboard.show_messages == 1:
        ...
        quote_icon = Image.open(os.path.join(picdir, "quote_icon_c.png"))
        ...
        ImageC.paste(quote_icon,(qx,qy),quote_icon)
        ...
        t_g, t_g, test_t_w, test_t_h = draw_color.textbbox((0,0),daily_message, font=font.SFQuote)
        ...
        if test_t_w > text_line_max:
            ...
            while l < ql:
                ...
                l += 1
                t_g, t_g, test_t_w, test_t_h = draw_color.textbbox((0,0),textbuffer, font=font.SFQuote)
                #print(textbuffer)
                if test_t_w >= text_line_max:
        ...
        tq_g, tq_g, tq_w, tq_h = draw_color.textbbox((0,0),text_line[qc], font=font.SFQuote)
        for i in text_line:
            tq_g, tq_g, tq_w, tq_h = draw_color.textbbox((0,0),i, font=font.SFQuote)
            q_h = tq_h
            if tq_w > q_w :
                q_w = tq_w
            qc +=1
        ...
        aqG, aqG, aq_w, aq_h = draw_color.textbbox((0,0),"- "+dashboard.quote.quote_author, font=font.SFQuoteAuthor)
        ...
        while qc < qs:
            tq_g, tq_g, tq_ww, tq_g = draw_color.textbbox((0,0),text_line[qc], font=font.SFQuote)
            gTx = screen.middle_w - int(tq_ww/2)
            draw_color.text((gTx, gTy), text_line[qc], font=font.SFQuote, fill=black)
            #print(text_line[qc])
            qc += 1
            gTy = gTy + int(q_h)
            #if qc == 1:
        ...
        qG, qG, q_w, q_h = draw_color.textbbox((0,0),"- "+dashboard.quote.quote_author, font=font.SFQuoteAuthor)
        gTx = int(screen.middle_w - q_w/2)
        gTy = gTy -2
        draw_color.text((gTx, gTy), "- "+dashboard.quote.quote_author, font=font.SFQuoteAuthor, fill=red)
    ...
    if dashboard.show_power == 1:
        ...
        if battlevel.level !=-1: #Checking if battery level can be goten -1 means error
            ...
            if battlevel.state == "Charging":
                ...
                refresh_mesasage = "Daily Quote, "+hourglass.next_refresh
    ...
    draw_color.text((gTx, gTy), refresh_mesasage, font=font.SFMonth, fill=black)
...
def daily_agile_quotes():
    ...
    if screen.epd_found == True:
        try:
            applog("Agile Quote Screen" ,"Setting screen")
            epd = epd7in3f.EPD()
            epd.init()
            applog("Agile Quote Screen" ,"INIT screen")
            if performance.cli == "noclean":
                applog("Agile Quote Screen" ,"Screen cleaning skipped")
            else:
                applog("Agile Quote Screen" ,"Time to clean the screen")
                epd.Clear()
            screen.height = epd.height
            screen.width = epd.width
            screen.middle_w = screen.width/2
            ...
            applog("Agile Quote Screen" ,"error"+str(e)) 

        except KeyboardInterrupt:
            applog("Agile Quote Screen" ,"ctrl + c received") 
            epd7in3e.epdconfig.module_exit()
            exit()
    else:
            screen.height = 480
    ...
    if dashboard.show_messages == 1:
        ...
        quote_icon = Image.open(os.path.join(picdir, "quote_icon_eri_c.png"))
    ...
    if dashboard.show_power == 1:
        ...
        if battlevel.level !=-1: #Checking if battery level can be goten -1 means error
            ...
            if battlevel.state == "Charging":
                ...
                hourglass.last_refresh = "Agile Quote, "+hourglass.last_refresh
                refresh_mesasage = "Agile Quote, "+hourglass.next_refresh
                applog("Dashoard","Show the last refresh text at the bottom, centered "+refresh_mesasage)
                t_G, t_G, test_t_w, test_t_h = draw_color.textbbox((0,0),refresh_mesasage, font=font.SFMonth)
    ...
    gTy = screen.height - int(test_t_h + 4)
    draw_color.text((gTx, gTy), refresh_mesasage, font=font.SFMonth, fill=black)
    #ImageC.save("agile_quote_debug.png")
...
def daily_dadjokes():
    ...
    if screen.epd_found == True:
        try:
            applog("Dad-Joke Screen" ,"Setting screen")
            epd = epd7in3f.EPD()
            epd.init()
            applog("Dad-Joke Screen" ,"INIT screen")
            if performance.cli == "noclean":
                applog("Dad-Joke Screen" ,"Screen cleaning skipped")
            else:
                applog("Dad-Joke Screen" ,"Time to clean the screen")
                epd.Clear()
            screen.height = epd.height
            screen.width = epd.width
            screen.middle_w = screen.width/2
            ...
            applog("Dad-Joke Screen" ,"error"+str(e)) 

        except KeyboardInterrupt:
            applog("Dad-Joke Screen" ,"ctrl + c received") 
            epd7in3e.epdconfig.module_exit()
            exit()
    else:
            screen.height = 480
    ...
    if dashboard.show_messages == 1 :
        ...
        if dashboard.show_dadjoke_live == 0:
            applog("Dad-Joke of the day" ,"File Joke db selected ")
            dad_joke_icon = Image.open(os.path.join(picdir, "dad_quote_c.png"))
        else:
            applog("Dad-Joke of the day" ,"LIVE Joke selected ")
            dad_joke_icon = Image.open(os.path.join(picdir, "dad_quotef_c.png"))
        ...
        ImageC.paste(dad_joke_icon,(qx,qy),dad_joke_icon)
        ...
        #See if the total is larger than the text_line_max value set.
        applog("Dad-Joke of the day" ,"Max pixels per line is "+str(text_line_max))
        t_g, t_g, test_t_w, test_t_h = draw_color.textbbox((0,0),daily_message, font=font.SFQuote)
        ...
        if test_t_w > text_line_max:
            ...
            applog("Dad-Joke of the day" ,"Dad-Joke fits in one row...")
            if len(daily_message):
                applog("Message of the day" ,"Using only one row here.")
                text_line.append(daily_message)
                t_g, t_g, test_t_w, test_t_h = draw_color.textbbox((0,0),daily_message, font=font.SFQuote)
        ...
        applog("Dad-Joke of the day","Total Dad-Joke height is "+str(quote_total_height))
        applog("Dad-Joke ",daily_message)
        ...
        ###########################
        # End of Dad Joke of the day
        ###########################
    else:
        applog("Dashboard","Dad Joke of the day feature : OFF")
    ...
    if dashboard.show_power == 1:
        ...
        if battlevel.level !=-1: #Checking if battery level can be goten -1 means error
            ...
            if battlevel.state == "Charging":
                ...
                hourglass.last_refresh = "Dad-Joke, "+hourglass.last_refresh
                refresh_mesasage = "Dad-Joke, "+hourglass.next_refresh
                applog("Dashoard","Show the last refresh text at the bottom, centered "+refresh_mesasage)
                t_G, t_G, test_t_w, test_t_h = draw_color.textbbox((0,0),refresh_mesasage, font=font.SFMonth)
...
def daily_comic():
    ...
    applog("OfficeInk" ,"Daily Image screen initiated")


    photodir = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'photos')
    picdir = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'icons')
    weatherdir = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'icons/weather')
    fontdir = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'fonts')
    ...
    #print(file_list)
    ph_n = len(file_list)
    pr_n = random.randint(0,(ph_n -1))
    print(str(len(file_list))+" Photos found")
    print("Selected photo :"+file_list[pr_n])
    
    photo = Image.open(os.path.join(photodir, file_list[pr_n]))
    x = 0
    y = 0

    #Setting a border around the edges of the screen
    sh = int(screen.height - 60)
    sw = int(screen.width - 60)
    photo = photo.convert('RGBA')

    photo = ImageOps.contain(photo, (sw, sh))
    print("Photo frame : photo is "+str(photo.size[0])+" by "+str(photo.size[1]))

    ## Print the Photo
    x = int(screen.middle_w - photo.size[0]/2)
    y = int(screen.middle_h - photo.size[1]/2)

    ImageC.paste(photo, (x,y))
    ...
    if dashboard.show_power == 1:
        ...
        if battlevel.level !=-1: #Checking if battery level can be goten -1 means error
            ...
            if battlevel.state == "Charging":
                ...
                hourglass.last_refresh = "Comic, "+hourglass.last_refresh
                refresh_mesasage = "Comic, "+hourglass.next_refresh
                applog("Dashoard","Show the last refresh text at the bottom, centered "+refresh_mesasage)
...
def crashlog(file_path:str, crash_message: str):
    ...
    applog("OfficeInk" ,"Logging crash message")
...
def logrotate(file_path:str):
    ...
    if os.path.exists(subdir) == False:
        try:
            ...
            print("Could not create log directory "+subdir+" Error: "+str(e))
    if os.path.exists(file_path):
        file_size = os.stat(file_path).st_size
        applog("System" ,"Log file exists, rotating...")
        new_file = file_path + ".old"
...
def sleep_screen(wake_date:date):
    ...
    ImageC = Image.new('RGB', (epd.width, epd.height), white)  # 255: clear the frame
    draw_color = ImageDraw.Draw(ImageC)
    sleep_icon = Image.open(os.path.join(picdir, 'sleep_icon_c.png'))

    sleep_string = "Good Night..."
    t_g, t_g, test_t_w, test_t_h = draw_color.textbbox((0,0),sleep_string, font=font.SleepFont)

    sX = int(screen.middle_w) - int(sleep_icon.size[0]/2)
    sY = int(screen.middle_h) - int(sleep_icon.size[1]/2)
    sY = sY - int(test_t_h)
    ImageC.paste(sleep_icon, (sX,sY), sleep_icon)

    sX = int(screen.middle_w) - int(test_t_w/2)
    sY = int(sY + sleep_icon.size[1]) + 4
    draw_color.text((sX, sY), sleep_string, font = font.SleepFont, fill = 'rgb(0,0,0)')

    sleep_string = "Screen will wakeup tomorrow at "+str(screen.wake_hour)+", sleep well!"
    t_g, t_g, test_t_w, test_t_h = draw_color.textbbox((0,0),sleep_string, font=font.SleepFont_foot)
    sX = int(screen.width - (int(test_t_w) + 5))
    sY = int(screen.height - (int(test_t_h) + 5))
    draw_color.text((sX, sY), sleep_string, font = font.SleepFont_foot, fill = 'rgb(0,0,0)')
...
def weekend_screen():
    ...
    if screen.epd_found == True:
        try:
            applog("Weekend Screen" ,"Setting screen")
            epd = epd7in3f.EPD()
            epd.init()
            applog("Weekend Screen" ,"INIT screen")
            if performance.cli == "noclean":
                applog("Weekend Screen" ,"Screen cleaning skipped")
            else:
                applog("Weekend Screen" ,"Time to clean the screen")
                epd.Clear()
            screen.height = epd.height
            screen.width = epd.width
            screen.middle_w = screen.width/2
    ...
    sleep_icon = Image.open(os.path.join(picdir, 'weekend_icon_c.png'))
            ...
            if battlevel.state == "Charging":
                ...
                if battlevel.level > 35:
                    ...
                    if battlevel.level < 85:
                        ...
                        batt_icon = Image.open(os.path.join(picdir, 'Battery_low_c.png'))
                        btx = 5
                        bty = (screen.heigh) - int(batt_icon.height)
                        bty = bty - 4
                        ImageC.paste(batt_icon, (btx,bty), batt_icon)
        else:
            applog("Dashboard","Battery not found!")
...
def gotosleep():
    applog("OfficeInk","Shutting down the host...")
    time.sleep(20)
...
def main():
    ...
    #performance.debug = 1
    logrotate("/home/noonas/FTP/OfficeInk/applog.log")
    ...
    if "noclean" in sys.argv:
        screen.clean_screen = -1
        print(date_time_stamp+" | No Clean screen")


    trend = 0
    applog("OfficeInk","Evening hour is set to: "+str(hourglass.evening_hour))
    if "nowelcome" in performance.cli:
        applog("OfficeInk","Skipping welcome screen...")
    else:
        welcome_screen(dashboard.delay_start_sec)

    if "wifi-off" in performance.cli:
        applog("OfficeInk","Turning WIFI OFF")
        wifi_off()
    else:
        applog("OfficeInk","WIFI Stays ON")


    while True :
        applog("OfficeInk","Screen sleep at: "+str(screen.sleep_hour))
        applog("OfficeInk","Wake up at: "+str(screen.wake_hour))
        min = 0
        ...
        hourglass.next_refresh = nextrefreshtime.strftime("Next Refresh @ %A %b %-d, %H:%M")
        hourglass.curenttime = int(datetime.now().strftime("%H"))
        hourglass.hour = int(datetime.now().strftime("%H"))
        performance.countr +=1
        applog("OfficeInk","it is now "+datetime.now().strftime("%H"))
        if datetime.now().strftime("%A") == "Monday" or datetime.now().strftime("%A") == "Tuesday" or datetime.now().strftime("%A") == "Wednesday":
            applog("OfficeInk","Time to draw the quote screen...")
            daily_agile_quotes()
        elif datetime.now().strftime("%A") == "Wednesday":
            applog("OfficeInk","Time to draw the quote screen...")
            daily_quotes()
        elif datetime.now().strftime("%A") == "Thursday" or datetime.now().strftime("%A") == "Friday":
            applog("OfficeInk","Time to draw the dad-joke screen...")
            daily_dadjokes()
        else:
            applog("OfficeInk","Its not a workday...")
            applog("OfficeInk","Time to go to sleep...")
            weekend_screen()
        if performance.keepalive == 0 and battery.state == "Not Charging":
            applog("System startup","Keepalive set to OFF or Battery is not charging")
            applog("OfficeInk","Running only once daily - Shutting down...")
            gotosleep()
        if performance.keepalive == 0 and battery.state == "Charging":
            applog("OfficeInk","Connected to Batterty, lets stay alive")
        else:
            applog("System startup","Keepalive set to ON")
        #applog("OfficeInk","Refresh in "+str(screen.refresh_rate_min)+" minutes")
        applog("OfficeInk: Main Loop",hourglass.next_refresh)
